...
import (
	...
	"github.com/dipsingh/gobgp/config"
	"github.com/dipsingh/gobgp/packet/bgp"
	"github.com/dipsingh/gobgp/table"
	"gopkg.in/tomb.v2"
	"io"
	"math/rand"
	"net"
...
func buildopen(gConf *config.Global, pConf *config.Neighbor) *bgp.BGPMessage {
	...
	return bgp.NewBGPOpenMessage(uint16(as), holdTime, gConf.Config.RouterId,[]bgp.OptionParameterInterface{opt})
}
	...
	if err != nil {
		...
		if h.fsm.state == bgp.BGP_FSM_ESTABLISHED {
			...
			case bgp.BGP_MSG_UPDATE:
				...
				//fmt.Println("In BGP MSG Update At the Bottom..")
	...
	for {
		...
		case o := <-h.outgoing.Out():
			...
			for _, msg := range table.CreateUpdateMsgFromPaths(m.Paths) {
				fmt.Println("Waiting for the channel",msg)
				fmt.Println("Waiting for the channel and M is",m)
