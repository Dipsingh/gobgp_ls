...
func (l *LabeledVPNIPAddrPrefix) Serialize() ([]byte, error) {
	fmt.Println("Labelled VPN Prefix called")
	buf := make([]byte, 1)
	buf[0] = l.Length
...
func (msg *BGPUpdate) Serialize() ([]byte, error) {
	...
	pbuf := make([]byte, 2)
	fmt.Println("MSG PathAttributes",msg.PathAttributes)
	for _, p := range msg.PathAttributes {
		fmt.Println("P is ",p)
		onepbuf, err := p.Serialize()
		fmt.Println("OnePuf Path",p)
		fmt.Println("OnePuf",onepbuf)
